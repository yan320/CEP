...
from energy_buffer import ReplayBufferEnergy
...
class SAC(object):
    ...
    def select_action(self, state, eval=False):
        ...
        if eval == False:
            action, _, _, _ = self.policy.module.sample(state)
        else:
            _, _, action, _ = self.policy.module.sample(state)
    ...
    def update_parameters(self, memory, batch_size):
        ...
        with torch.no_grad():
            ...
            next_state_action, next_state_log_pi, _, _ = self.policy.module.sample(next_state_batch)
        ...
        # qf_loss = normalized_weights * (torch.max(qf1_loss, qf2_loss)).mean()
        self.critic_optim.zero_grad()
        qf_loss.backward()
        self.critic_optim.step()

        pi, log_pi, mean, log_std = self.policy.module.sample(state_batch)

        qf1_pi, qf2_pi = self.critic(state_batch, pi)
        min_qf_pi = torch.min(qf1_pi, qf2_pi)

        policy_loss = ((self.alpha * log_pi) - min_qf_pi).mean()
        # policy_loss = normalized_weights * ((self.alpha * log_pi) - min_qf_pi).mean()
...
ACTION_W_MIN = -1.82  # rad/s
ACTION_V_MAX = 0.26  # m/s
ACTION_W_MAX = 1.82  # rad/s
world = 'scf_4'
replay_buffer_size = 100000

replay_buffer = ReplayBufferEnergy(replay_buffer_size)
agent = SAC(state_dim, action_dim)
replay_buffer_len = len(replay_buffer)
k = 1 + replay_buffer_len / replay_buffer_size
batch_size = int(k * batch_size)

agent.load_models(1800)
...
if __name__ == '__main__':
    ...
    for ep in range(max_episodes):
        ...
        episode = []
        succ = 0
        delta = (end_lam - init_lam) / (max_episodes - 1)
        if ep > before_training:
            lam += delta

        if is_training and not ep % 10 == 0 and len(replay_buffer) > before_training:
            print('Episode: ' + str(ep) + ' training')
        else:
            if len(replay_buffer) > before_training:
        ...
        for step in range(max_steps):
            ...
            goal_angle = math.degrees(state[-2])
            obstacle = min(state[:10])
            velocity, angular_velocity = FL.get_avoidance_output(obstacle, env.angle, goal_angle)
            fl_action = np.array([velocity, angular_velocity])
            ...
            action0 = 0.6 * action[0] + 0.4 * fl_action[0]
            action1 = 0.8 * action[1] + 0.2 * fl_action[1]

            unnorm_action = np.array([action_unnormalized(action0, ACTION_V_MAX, ACTION_V_MIN),
                                      action_unnormalized(action1, ACTION_W_MAX, ACTION_W_MIN)])


            next_state, reward, done, goal = env.step(unnorm_action, past_action)
            if goal:
                succ += 1
            ...
            if not ep % 10 == 0 or not len(replay_buffer) > before_training:
                if reward == 100.:
                    print('***\n-------- Maximum Reward ----------\n****')
                    for _ in range(3):
        ...
        if lam > 1:
            lam = 1
        trajectory_energy = replay_buffer.calculate_traj_energy(episode)
        priority = replay_buffer.self_paced_prioritized_curriculum(trajectory_energy, lam)
        if not len(episode) == 0:
            replay_buffer.push([episode, priority])

        update_times = int(k * step)
        for _ in range(update_times):
            if len(replay_buffer) > before_training and is_training:
        ...
        succ_rate = sum(success) / (ep + 1)

        mean_success_rate.append(succ_rate)
        # sample_num_list.append(sample_num)
        # if succ_rate >= 1.0 and not_go == False:
        #     print("achieving the success rate need time step:", step_num)
        #     not_go = True

        # moving_success.append(sum(success) / max_episodes+1e-5)
        print('reward per ep: ' + str(rewards_current_episode))
        print('rewards_current_episode: ' + str(rewards_current_episode) + ' and break step: ' + str(step))
        if ep % 10 == 0:
            if len(replay_buffer) > before_training:
