...
from fuzzy_control import get_avoidance_output
...
if __name__ == '__main__':
    ...
    for ep in range(max_episodes):
        ...
        #update_times = int(k * max_steps)
        for step in range(max_steps):
            state = np.float32(state)
            goal_angle = math.degrees(state[-2])
            obstacle = min(state[:10])
            velocity, angular_velocity = get_avoidance_output(obstacle, env.angle, goal_angle)
            ...
            # print("s_a:", unnorm_action[0])
            # print("f_a:", velocity)
            # print("****************************")
            # print("s_w:", unnorm_action[1])
            # print("f_w:", angular_velocity)
            unnorm_action[0] = 0.5 * unnorm_action[0] + 0.5 * fl_action[0]
            unnorm_action[1] = 0.8 * unnorm_action[1] + 0.2 * fl_action[1]
            # print("****************************")
            # print("a:", unnorm_action[0])
            # print("w:", unnorm_action[1])
            # print("****************************")
            unnorm_FL_action = np.array([action_unnormalized(unnorm_action[0], ACTION_V_MAX, ACTION_V_MIN),
                                      action_unnormalized(unnorm_action[1], ACTION_W_MAX, ACTION_W_MIN)])

            next_state, reward, done, goal = env.step(unnorm_FL_action, past_action)
            if goal:
                succ += 1
