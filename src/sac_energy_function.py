...
if __name__ == '__main__':
    ...
    for ep in range(max_episodes):
        ...
        for _ in range(step+1):
            if len(replay_buffer) > before_training and is_training and not ep % 10 == 0:
        ...
        if not os.path.exists(directory):
            os.mkdir(directory)
        file_path = os.path.join(directory, 'average_rewards_hard.txt')
        with open(file_path, 'w') as file:
            for item in average_rewards:
                file.write(str(item) + '\n')

        file_path = os.path.join(directory, 'success_of_episode_hard.txt')
        with open(file_path, 'w') as file:
