...
with open(filename+'/mean_reward_list.txt','r') as file:
    lines1 = file.readlines()
mean_reward_list = [float(line.strip()) for line in lines1]

with open(filename+'/mean_success_rate.txt','r') as file:
...
if __name__ == '__main__':
    ...
    for ep in range(MAX_EPISODES):
        ...
        if not os.path.exists(directory):
            os.mkdir(directory)
        file_path = os.path.join(directory, 'mean_reward_list.txt')
        with open(file_path, 'w') as file:
            for item in mean_reward_list:
                file.write(str(item) + '\n')

        file_path = os.path.join(directory, 'mean_success_rate.txt')
        with open(file_path, 'w') as file:
