...
MAX_STEPS = 1000
MAX_BUFFER = 200000
filename = os.path.expanduser('~/.ros/DDPG1')
with open(filename+'/mean_reward_list_4.txt','r') as file:
    lines1 = file.readlines()
mean_reward_list = [float(line.strip()) for line in lines1]

with open(filename+'/mean_success_rate_4.txt','r') as file:
    lines2 = file.readlines()
mean_success_rate = [float(line.strip()) for line in lines2]
with open(filename+'/rewards_all_episodes_4.txt','r') as file:
    lines3 = file.readlines()
rewards_all_episodes = [float(line.strip()) for line in lines3]

with open(filename+'/success_num_lsit_4.txt','r') as file:
    lines4 = file.readlines()
success_num_lsit = [float(line.strip()) for line in lines4]
...
noise = OUNoise(ACTION_DIMENSION, max_sigma=0.1, min_sigma=0.1, decay_period=1000000)
trainer.load_models(1860)
...
if __name__ == '__main__':
    ...
    for ep in range(1879, MAX_EPISODES):
        succ = 0
        done = False
        state = env.reset()
        ...
        for step in range(MAX_STEPS):
            ...
            if done or step == MAX_STEPS - 1:
                ...
                mean_reward_list.append(sum(rewards_all_episodes) / (ep + 1))
        ...
        success_num_lsit.append(succ)
        succ_rate = sum(success_num_lsit) / (ep + 1)
        mean_success_rate.append(succ_rate)

        directory = 'DDPG'
        if not os.path.exists(directory):
            os.mkdir(directory)
        file_path = os.path.join(directory, 'mean_reward_list_4.txt')
        with open(file_path, 'w') as file:
            for item in mean_reward_list:
                file.write(str(item) + '\n')

        file_path = os.path.join(directory, 'mean_success_rate_4.txt')
        with open(file_path, 'w') as file:
            for item in mean_success_rate:
                file.write(str(item) + '\n')

        file_path = os.path.join(directory, 'rewards_all_episodes_4.txt')
        with open(file_path, 'w') as file:
            for item in rewards_all_episodes:
                file.write(str(item) + '\n')
        file_path = os.path.join(directory, 'success_num_lsit_4.txt')
        with open(file_path, 'w') as file:
            for item in success_num_lsit:
                file.write(str(item) + '\n')

    # plot
    # print("success:", sum(success) / max_episodes)
    # print("success_number:", sum(success))
